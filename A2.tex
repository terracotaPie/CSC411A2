...
\begin{homeworkProblem}
	...
	To construct the neural network, we trained on \emph{ENTER NUMBER HERE} 90?
	images per actor. Each image was cropped to a bounding box that contained only the actor's face. Images were serialized in a dictionary that mapped the actors' names to an array list of their whole RGB images saved in a numpy array.$NOTE: DOUBLE CHECK THAT IT WAS A NUMPY ARRAY?$ 
	...
	This dictionary was read from a file called \texttt{actor_imgs.p} using the \texttt{pickle} module to create a unique training, validation and test set.
	...
	To preprocess the images we did the following to each:
	\begin{itemize}
		\item grayscaling the image to convert the RGB image to a 2D array \texttt{rgb2gray()} function given in class.
	...
	\bold{Neural Network Performance}
